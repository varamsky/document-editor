...
export default function Version() {
  ...
  const [selectedVersionData, setSelectedVersionData] = useState<IElement[]>(
    []
  );
      ...
      if (documentId) {
        ...
        if (response.data.success && response.data.total > 0) {
          const versionArr: VersionType[] = response.data.data.map(
            (item: VersionType) => ({
              ...item,
              time: formatDate(item.time, true),
            })
          );
  ...
  useEffect(() => {
    ...
    DOMEventHandlers.register(container, selectedVersionData, editorOptions);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (documentId) {
      DOMEventHandlers.setContent({ main: selectedVersionData });
    }
  }, [documentId, selectedVersionData]);
    ...
    <>
      <Box sx={{ display: "flex", backgroundColor: "#f9fbfd" }}>
        ...
        >
          ...
          /> */}
          <div className="canvas-editor-version editor" />
        </Box>
        <Drawer
          sx={{
            width: drawerWidth,
